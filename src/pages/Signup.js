import React, { useState } from 'react';
import { Button, Form, Grid, Header, Segment } from 'semantic-ui-react'
import CONSTANTS from '../constants';

const Signup = () => {

  const [newUser, setNewUser] = useState({ name: '', email: '_@_.com', password: 'shred420', budget: 0, fridges: [] });

  // TODO: finish helping

  //handlers/helpers for account form 
  const handleNewUserFormSubmit = () => {
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(state.newUser),
    }

    fetch(CONSTANTS.USERS_URL, config)
      .then(res => res.json())
      .then(newUser => props.updateCurrentUser(newUser, props))
  }

  const handleNewUserFormChange = (e, val) => {
    const { name, value } = val;
    //TODO: FIX THIS!!!!!!!!!!!!!!!
    setState((prevState) => {
      prevState.newUser[name] = value;
      return prevState;
    })
  }

  return (
    <Grid textAlign='center' style={{ height: '100vh' }} verticalAlign='middle'>
      <Grid.Column style={{ maxWidth: '50vh' }}>
        <Segment>
          <Form size='large' onSubmit={handleNewUserFormSubmit}>
            <Header as='h2' color='blue' textAlign='left'>
              Create Account
              </Header>
            <Form.Input
              label="Email"
              name='email'
              fluid
              icon='mail'
              iconPosition='left'
              onChange={handleNewUserFormChange}
            />
            <Form.Input
              label="Name"
              name='name'
              fluid
              icon='user'
              iconPosition='left'
              onChange={handleNewUserFormChange}
            />
            <Form.Input
              label="Password"
              name='password'
              fluid
              icon='lock'
              iconPosition='left'
              type='password'
              onChange={handleNewUserFormChange}
            />
            <Form.Input
              label="Budget"
              name='budget'
              fluid
              icon='dollar sign'
              iconPosition='left'
              onChange={handleNewUserFormChange}
            />
            <Button color='teal' fluid size='large' type='submit'>
              Submit
              </Button>
          </Form>
        </Segment>
      </Grid.Column>
    </Grid>
  )
}

export default Signup;
