...
const FridgeDetail = (props) => {
  ...
  const handleFoodFormSubmit = () => {
    ...
    if (state.newFood.is_drink) {
      if (
        !fridgeCapacity.drink.full &&
        fridgeCapacity.drink.total + newFood_quantity <=
          state.currentFridge.drink_capacity
      ) {
      ...
      if (
        !fridgeCapacity.food.full &&
        fridgeCapacity.food.total + newFood_quantity <=
          state.currentFridge.food_capacity
      ) {
        fetch(BASE_API_URL + "/food_items/create", config)
          .then((res) => res.json())
          .then(({ food_item }) => addFoodToCurrentFridge(food_item));
  ...
  const addFoodToCurrentFridge = (foodItem) => {
    actions.setCurrentFridge({
      ...state.currentFridge,
      food_items: [...state.currentFridge.food_items, foodItem],
    });
    setCurrentFoodItemsNearExpiry(); //TODO: remove? do we need if using checkbox?
  ...
  const removeFoodFromCurrentFridge = (foodItem) => {
    actions.setCurrentFridge({
      ...state.currentFridge,
      food_items: state.currentFridge.food_items.filter(
  ...
  const handleFoodItemDelete = (e, foodItem) => {
    ...
    fetch(BASE_API_URL + FOOD_DELETE_URL(foodItem.id), config).then(
  ...
  const checkFoodItemsNearExpiry = async (foodItem) => {
    ...
    const response = await fetch(
      BASE_API_URL + `/food_items/${foodItem.id}/check_if_expiring`,
      config
    );
  ...
  const setCurrentFoodItemsNearExpiry = async () => {
    ...
    return await Promise.all(
      state.currentFridge.food_items.filter((foodItem) => {
  ...
  const getRecipesForFoodItemsNearExpiry = async () => {
    ...
    if (expiringFoodItems.length === 0) {
      ...
      console.log(state.checkedFood);
      const foodNameString = state.checkedFood.join(",");
  ...
  const fetchRecipes = async (numOfRecipes = 3, foodNameString) => {
    ...
    const res = await fetch(
      SPOONACULAR_URL +
        `/recipes/findByIngredients?ingredients=${foodNameString}&number=${numOfRecipes}`,
      config
    );
  ...
  return (
    ...
    >
      <Grid.Column style={{ maxWidth: "100vh" }}>
        <Card.Group centered>{displayFridge()}</Card.Group>
        <Card.Group centered>{displayFoodItems()}</Card.Group>
      </Grid.Column>
    </Grid>
  );
};
