...
const FoodDetailsGraphs = (props) => {
  ...
  const calculateTimeUntilExpiry = (expiryDate, inDays = true) => {
    ...
    if (inDays === true) {
      ...
      if (timeTilExpiry < 0) return 0;
      else return timeTilExpiry;
    } else {
  ...
  const populateBarChart = () => {
    return props.currentFridge.food_items.map((foodItem) => {
      return {
        x: foodItem.name,
        y: calculateTimeUntilExpiry(foodItem.expiration_date),
      };
    });
  };
      ...
      <Card.Content>
        <Segment>
          ...
          <VictoryChart domainPadding={30}>
            <VictoryLabel
              text="Days Left Until Expiration"
              x={225}
              y={30}
              textAnchor="middle"
            />
        ...
        <Statistic.Group widths="two">
          <Statistic>
            <Statistic.Value>
              {" "}
              {calculateTotalFoodAndDrink().drinkCount} /{" "}
              {props.currentFridge.drink_capacity}
            </Statistic.Value>
            <Statistic.Label>Drink Capacity</Statistic.Label>
          </Statistic>
          <Statistic>
            <Statistic.Value>
              {calculateTotalFoodAndDrink().foodCount} /{" "}
              {props.currentFridge.food_capacity}
            </Statistic.Value>
        ...
        <RecipeApiFetcher
          getRecipesForFoodItemsNearExpiry={
            props.getRecipesForFoodItemsNearExpiry
          }
          recipes={props.recipes}
          foodItems={props.currentFridge.food_items}
        />
      </Card.Content>
      <Card.Content extra>
        Location: Seattle, WA
